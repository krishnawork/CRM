...
import Billing_details from "../billing/Billing_details.js";
...
class Payment extends React.Component {
  ...
  createSelectItemsPatients = (val) => {
    ...
    for (let i = 0; i < this.props.remaining_bill_patients.length; i++) {
      ...
      items.push(
        <option
          key={this.props.remaining_bill_patients[i].id}
          value={this.props.remaining_bill_patients[i].id}
        >
          {this.props.remaining_bill_patients[i].first_name}{" "}
          {this.props.doctors[i].last_name}
        </option>
      );
    }
  ...
  getPatients = () => {
    ...
    for (let i = 0; i < this.props.remaining_bill_patients.length; i++) {
      //console.log(i);
      if (
        this.props.remaining_bill_patients[i].userID != 0 &&
        this.props.remaining_bill_patients[i].amount !=
          this.props.remaining_bill_patients[i].totalAmount
      )
        items.push(
          <option
            key={this.props.remaining_bill_patients[i].id}
            value={this.props.remaining_bill_patients[i].userID}
          >
            {this.get_user_name(this.props.remaining_bill_patients[i].userID)}
          </option>
        );
    }
  ...
  getBillingNumber = () => {
    ...
    for (let i = 0; i < this.props.bill_numbers.length; i++) {
      //console.log(i);
      if (
        this.props.bill_numbers[i].id != 0 &&
        this.props.bill_numbers[i].amount !=
          this.props.bill_numbers[i].totalAmount
      )
        items.push(
          <option
            key={this.props.bill_numbers[i].id}
            value={this.props.bill_numbers[i].id}
          >
            {this.props.bill_numbers[i].id}
          </option>
        );
    }
  ...
  submit = (event) => {
    ...
    if (this.state.orderBillId == 0) {
      ...
      if (
        this.state.orderBillId != 0 &&
        this.state.orderBillId != 0 &&
        this.state.payAmount != 0
      ) {
        this.props.dispatch(
          createBill({
            id: this.state.orderBillId,

...
class Appoiment extends React.Component {
  ...
  onEdit = (event, row) => {
    ...
    var formated_Date = `${date.getFullYear()}-${date.getMonth() + 1}-${
      date.getDate() - 1
    }`;
    ...
    return (
      <div className={s.root}>
        ...
        <Row>
          <Col xl={12}>
            {appoinments.length > 0 ? (
              <Widget title={<p style={{ fontWeight: 700 }}>Appointment</p>}>
                <Table responsive>
                  <thead>
                    <tr className="fs-sm">
                  ...
                  <tbody>
                    {appoinments.map((row) => (
                      <tr key={row.id}>
                        <td>{row.id}</td>
                        <td>{this.get_user_name(row.patientID)}</td>
                        <td>{this.get_doctor_name(row.doctorID)}</td>
                        <td>{this.convertDateFormate(row.date)}</td>
                        <td>{row.time}</td>
                        ...
                        <td>
                          ...
                          <a onClick={(event) => this.onDelete(event, row.id)}>
                            <img
        ...
        <Modal isOpen={this.state.modal} toggle={this.toggle}>
          <ModalHeader toggle={this.toggle}>
            {this.state.id == 0 ? "Create Appointment" : "Edit Appointment"}
          </ModalHeader>
          <ModalBody>
            ...
            <div className="form-group">
              ...
              <Input
                type="select"
                onChange={(e) => this.onDropdownSelectedDoctors(e)}
                label="Doctors"
            ...
            <div className="form-group">
              <label htmlFor="Doctors">Time</label>
              <Input
                type="select"
                onChange={(e) => this.onDropdownShiftItemsForDoctors(e)}
                label="Doctors"
          ...
          <ModalFooter>
            <Button
              color="primary"
              onClick={(event) => this.createAppoinment(event)}
            >
